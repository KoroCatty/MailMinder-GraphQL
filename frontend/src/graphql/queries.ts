...
export const GET_POSTS_BY_ID = gql`
  query getPostsById($uid: ID!, $first: Int, $skip: Int) {
    # getPostsById の命名は何でもok
    PostsByUser(id: $uid, first: $first, skip: $skip) {
      # resolver で定義した名前
...
export const GET_POSTS_BY_ID_LIMIT = gql`
  query getPostsByIdLimit($uid: ID!, $limit: Int!) {
    # getPostsById の命名は何でもok
    PostsByUserLimit(id: $uid, limit: $limit) {
      # resolver で定義した名前
      id
      title
      content
