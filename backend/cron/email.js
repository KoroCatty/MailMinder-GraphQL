  ...
  async () => {
    try {
      ...
      for (const user of allUsers) {
        ...
        // 4. E メールの本文を組み立てる
        const attachments = [];
        const htmlContent = selectedPosts
          .map((post, index) => {
            let imgTag;
        ...
        // E メールの内容を定義 (Demoユーザー ユーザーにはメールを送らない)
        if (user.email !== "demo@demo.com") {
          ...
          console.log(
            `Email sent to ${user.email}: ${info.response}`.cyan.bold.underline,
          );
        } else {
          console.log(
            `Skipped sending email to demo user: ${user.email}`.cyan.bold
              .underline,
          );
        }
      }
